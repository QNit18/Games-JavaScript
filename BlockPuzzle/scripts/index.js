...
class Board {
    ...
    handleCompleteRows(){
        const lastedGrid = board.grid.filter((row) => { // row => []
            return row.some(col => col === WHITE_COLOR_ID);
        });

        // Tổng cộng hàng đã hoàn thành
        const newScore = ROWS - lastedGrid.length;
        const newRows = Array.from({ length: newScore }, () => Array(COLS).fill(WHITE_COLOR_ID));

        if(newScore){
            board.grid = [...newRows, ...lastedGrid];
...
class Brick {
    constructor(id) {
        this.id = id; // Hình dang chung của hình
        this.layout = BRICK_LAYOUT[id];
        this.activeIndex = 0; // Biến đổi của hình
        this.colPos = 4;
        this.rowPos = -2;
    }
    ...
    // Xoá đi hình hiện tại để vẽ hình mới
    clear() {
        for (let row = 0; row < this.layout[this.activeIndex].length; row++) {
            for (
                let col = 0;
